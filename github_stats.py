...
class Queries(object):
    ...
    def user_stats() -> str:
        """
        :return: GraphQL query for user profile stats (PRs, issues, followers, etc.)
        """
        return """
query {
  viewer {
...
class Stats(object):
    ...
    def __init__(
        ...
        self._user_stats_fetched: bool = False
...
Languages:
  - {formatted_languages}"""
    ...
    async def lines_changed(self) -> Tuple[int, int]:
        """
        Skipped - this API call is too slow and unreliable (returns 202s)
    ...
    async def get_user_stats(self) -> None:
        """
        Fetch user profile stats (PRs, issues, followers, etc.)
        """
        if self._user_stats_fetched:
            return

        result = await self.queries.query(Queries.user_stats())
        ...
        self._user_stats_fetched = True

    @property
    async def pull_requests(self) -> int:
        """
        :return: total number of pull requests opened
        """
        if self._pull_requests is None:
            await self.get_user_stats()
        return self._pull_requests or 0

    @property
    async def issues(self) -> int:
        """
        :return: total number of issues opened
        """
        if self._issues is None:
            await self.get_user_stats()
        return self._issues or 0

    @property
    async def pr_reviews(self) -> int:
        """
        :return: total number of PR reviews given
        """
        if self._pr_reviews is None:
            await self.get_user_stats()
    ...
    async def followers(self) -> int:
        ...
        if self._followers is None:
            await self.get_user_stats()
        return self._followers or 0

    @property
    async def years_active(self) -> int:
        """
        :return: number of years with contributions on GitHub
        """
        if self._years_active is None:
            await self.get_user_stats()
        return self._years_active or 0

    @property
    async def account_created(self) -> str:
        """
        :return: account creation date (ISO format)
        """
        if self._account_created is None:
            await self.get_user_stats()
        return self._account_created or ""

    @property
    async def member_since(self) -> str:
        """
        :return: year the account was created
        """
        created = await self.account_created
        if created:
            return created[:4]  # Extract year from ISO date
        return "N/A"
